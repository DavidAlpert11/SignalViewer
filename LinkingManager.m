classdef LinkingManager < handle
    ...
    methods
        ...
        function showLinkingDialog(obj)
            % Enhanced linking configuration dialog with better UI
            d = dialog('Name', 'CSV Linking Manager', 'Position', [150 100 700 550], 'Resize', 'on');

            % Title with icon
            uicontrol('Parent', d, 'Style', 'text', 'Position', [20 510 660 30], ...
                'String', '🔗 CSV Linking Manager', ...
                'FontSize', 16, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');

            % Description
            uicontrol('Parent', d, 'Style', 'text', 'Position', [20 475 660 25], ...
                'String', 'Link CSV files: When you assign a signal from one CSV, the same signal from linked CSVs is automatically assigned.', ...
                'FontSize', 10, 'HorizontalAlignment', 'left', 'ForegroundColor', [0.3 0.3 0.3]);

            % Left panel: Available CSVs with checkboxes
            uicontrol('Parent', d, 'Style', 'text', 'Position', [20 440 300 20], ...
                'String', '📁 Available CSV Files:', 'FontWeight', 'bold', 'FontSize', 11);
            
            % Create scrollable panel for CSV checkboxes
            csvPanel = uipanel('Parent', d, 'Position', [20 200 300 240], ...
                'Title', 'Select CSVs to Link', 'FontSize', 10, 'BorderType', 'etchedin');
            
            availableNodes = obj.getAvailableNodes();
            csvCheckboxes = cell(length(availableNodes), 1);
            
            % Use listbox with checkboxes simulation for better scrolling
            if isempty(availableNodes) || strcmp(availableNodes{1}, 'No CSV files loaded')
                uicontrol('Parent', csvPanel, 'Style', 'text', ...
                    'Position', [10 100 280 40], ...
                    'String', 'No CSV files loaded.\nPlease load CSV files first.', ...
                    'FontSize', 9, 'ForegroundColor', [0.6 0.6 0.6], ...
                    'HorizontalAlignment', 'center');
                csvCheckboxes = {};
            else
                % Create a listbox that allows multiple selection (simulates checkboxes)
                csvListbox = uicontrol('Parent', csvPanel, 'Style', 'listbox', ...
                    'Position', [10 10 280 210], ...
                    'String', availableNodes, ...
                    'Max', length(availableNodes), ...
                    'FontSize', 9);
                
                % Store reference for later use
                csvCheckboxes = {csvListbox};
            end

            % Right panel: Current link groups
            uicontrol('Parent', d, 'Style', 'text', 'Position', [340 440 340 20], ...
                'String', '🔗 Current Link Groups:', 'FontWeight', 'bold', 'FontSize', 11);
            
            groupsPanel = uipanel('Parent', d, 'Position', [340 200 340 240], ...
                'Title', 'Linked Groups', 'FontSize', 10, 'BorderType', 'etchedin');
            
            groupsListbox = uicontrol('Parent', groupsPanel, 'Style', 'listbox', ...
                'Position', [10 10 320 210], ...
                'String', obj.getLinkGroupsText(), 'FontSize', 9, ...
                'Max', 10, 'Enable', 'on');

            % Action buttons
            createBtn = uicontrol('Parent', d, 'Style', 'pushbutton', ...
                'Position', [20 160 140 30], ...
                'String', '➕ Create Link Group', ...
            ...
            uicontrol('Parent', d, 'Style', 'text', 'Position', [20 90 660 20], ...
                'String', '💡 Tip: Select 2 or more CSVs and click "Create Link Group" to link them together.', ...
                'FontSize', 9, 'ForegroundColor', [0.5 0.5 0.5], 'FontStyle', 'italic');

            % Bottom buttons
            clearBtn = uicontrol('Parent', d, 'Style', 'pushbutton', ...
            ...
            closeBtn = uicontrol('Parent', d, 'Style', 'pushbutton', ...
                'String', 'Close', ...
                'Position', [580 20 80 30], ...
                'FontSize', 10, 'FontWeight', 'bold', ...
                'Callback', @(~,~) close(d));
        end
        
        function createLinkGroupFromCheckboxes(obj, checkboxes, groupsListbox, dialog)
            % Create link group from selected listbox items
            if isempty(checkboxes) || isempty(checkboxes{1})
                uialert(obj.App.UIFigure, 'No CSV files available to link.', 'No Files', 'Icon', 'warning');
                return;
            end
            
            csvListbox = checkboxes{1};
            selectedIndices = csvListbox.Value;
            
            if isempty(selectedIndices) || length(selectedIndices) < 2
                uialert(obj.App.UIFigure, 'Please select at least 2 CSV files to create a link group.', 'Selection Required', 'Icon', 'warning');
                return;
            end
            
            % Create the link group
            obj.createLinkGroupFromIndices(selectedIndices);
            
            % Refresh the groups listbox
            groupsListbox.String = obj.getLinkGroupsText();
            
            % Clear selection
            csvListbox.Value = [];
            
            % Show success message
            obj.App.StatusLabel.Text = sprintf('✅ Created link group with %d CSVs', length(selectedIndices));
            obj.App.StatusLabel.FontColor = [0.2 0.6 0.9];
        end
        
        function deleteSelectedLinkGroup(obj, groupsListbox, dialog)
            % Delete the selected link group
            if isempty(obj.LinkedGroups)
                uialert(obj.App.UIFigure, 'No link groups to delete.', 'No Groups', 'Icon', 'info');
                return;
            end
            
            selectedIdx = groupsListbox.Value;
            if isempty(selectedIdx) || selectedIdx < 1 || selectedIdx > length(obj.LinkedGroups)
                uialert(obj.App.UIFigure, 'Please select a link group to delete.', 'Selection Required', 'Icon', 'warning');
                return;
            end
            
            % Confirm deletion
            groupName = obj.LinkedGroups{selectedIdx}.Name;
            answer = uiconfirm(obj.App.UIFigure, ...
                sprintf('Delete link group "%s"?', groupName), ...
                'Confirm Deletion', ...
            ...
            if strcmp(answer, 'Delete')
                obj.LinkedGroups(selectedIdx) = [];
                groupsListbox.String = obj.getLinkGroupsText();
                if ~isempty(groupsListbox.String)
                    groupsListbox.Value = min(groupsListbox.Value, length(groupsListbox.String));
                end
                obj.App.StatusLabel.Text = sprintf('🗑️ Deleted link group: %s', groupName);
                obj.App.StatusLabel.FontColor = [0.2 0.6 0.9];
            end
        end
        
        function clearAllLinksAndRefresh(obj, groupsListbox, dialog)
            % Clear all links and refresh the dialog
            obj.clearAllLinks();
            if ~isempty(groupsListbox)
                groupsListbox.String = obj.getLinkGroupsText();
            end
        end
