...
class SignalOperationsManager:
    ...
    def compute_derivative(self, signal_name: str, method: str = 'gradient',
                          smoothing: bool = False, window_size: int = 5) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def compute_integral(self, signal_name: str, method: str = 'trapezoidal',
                        initial_value: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def compute_operation(self, operation: str, signal1_name: str, 
                         signal2_name: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if operation == 'add':
            ...
            result = np.divide(data1_interp, data2_interp, 
    ...
    def interpolate_to_common_time(self, time1: np.ndarray, data1: np.ndarray,
                                   time2: np.ndarray, data2: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.interpolation_method == 'linear':
            ...
            f1 = interpolate.interp1d(time1, data1, kind=self.interpolation_method,
                                     bounds_error=False, fill_value='extrapolate')
            f2 = interpolate.interp1d(time2, data2, kind=self.interpolation_method,
                                     bounds_error=False, fill_value='extrapolate')
    ...
    def get_signal_data(self, signal_name: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if signal_name in self.derived_signals:
            ...
            return signal_data.get('time', np.array([])), signal_data.get('data', np.array([]))
        
        # Otherwise, it should be in data manager
        # This would need to be implemented based on how signals are identified
        return np.array([]), np.array([])
    
    def add_derived_signal(self, signal_name: str, time_data: np.ndarray, 
                          signal_data: np.ndarray, operation_info: Dict):
